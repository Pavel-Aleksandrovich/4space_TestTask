...
class ShopImpl: Shop {
    ...
    private var dic: [String: Product] = [:]
    
    func addNewProduct(product: Product) -> Bool {
        let result = dic[product.id] == nil
    ...
    func deleteProduct(id: String) -> Bool {
        let result = dic[id] == nil
        
        dic.removeValue(forKey: id)
    ...
    func listProductsByName(searchString: String) -> Set<String> {
        Set(dic
            ...
            .map { (key, value) in
                dic.filter { $0.value.name == value.name }.count > 1 ? "\(value.producer) - \(value.name)" : value.name
            }
            .prefix(10))
    }
    ...
    func listProductsByProducer(searchString: String) -> [String] {
        Array(dic
            ...
            .sorted { $0.value.producer < $1.value.producer }
            .map { $0.value.name }
            .prefix(10))
    }
}
